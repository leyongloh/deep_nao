...
class Nodo(object):
    def __init__(self):
        ...
        self.pub = rospy.Publisher('aruco_marker_position',Marker, latch=True)

    def callback(self, msg):
        rospy.loginfo('Image received...')

        self.image = self.br.imgmsg_to_cv2(msg, desired_encoding ="bgr8")
        #cv2.imshow("Top image", self.image)
        ...
        #print("Publish aruco:", self.p_aruco[0])
        # print("ids_aruco", ids_aruco)
        # aruco_msg = Marker()
        # aruco_msg.header.frame_id = "aruco_marker"
        # aruco_msg.id = str(ids)
        # aruco_msg.type = Marker.SPHERE
        # aruco_msg.action = Marker.ADD
        # aruco_msg.pose.position.x = self.p_aruco[0]
        # aruco_msg.pose.position.y = self.p_aruco[1]
        # aruco_msg.pose.position.z = self.p_aruco[2]
    ...
    # Compute Manually the static homogeneous transformation of
    ...
    def aruco_marker_wrt_torsoframe(self, ids):
        ...
        #self.pub.publish(ids, self.p_aruco[0], self.p_aruco[1], self.p_aruco[2])
        aruco_msg = Marker()
        aruco_msg.header.frame_id = "aruco_marker"
        aruco_msg.id = ids
        aruco_msg.type = Marker.SPHERE
